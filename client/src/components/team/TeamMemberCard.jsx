...
export default function TeamMemberCard({ member, isSelected, onSelect, onResend, resending, canManage = true }) {
  return (
    <button
      ...
      className={`w-full text-left bg-white dark:bg-gray-900 rounded-2xl border px-4 py-3.5 transition ${isSelected ? 'border-purple-300 ring-2 ring-purple-50 dark:ring-purple-500/20' : 'border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700'}`}
    >
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50 dark:border-gray-800">
        ...
        {member.isPending && canManage ? (
          <span
            ...
            className="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 text-xs font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition"
          >
          ...
          <span className="text-xs text-gray-400 dark:text-gray-500">Joined {formatDate(member.joined_at)}</span>
        )}
      </div>
    </button>
  )
