    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="px-5 pb-5">
        <div className="flex flex-col sm:flex-row sm:items-end gap-4 -mt-10">
          ...
          <div className="sm:ml-auto sm:pb-1 flex items-center gap-1.5 text-xs text-gray-400">
            <span className="material-icons" style={{ fontSize: '16px' }}>calendar_today</span>
            Joined {formatJoinDate(user?.created_at)}
          </div>
        </div>
        {user?.bio && <p className="text-sm text-gray-600 mt-4 leading-relaxed">{user.bio}</p>}
      </div>
    </div>
  )
}
