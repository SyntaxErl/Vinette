      ...
      <div className="max-w-7xl mx-auto w-full px-1 sm:px-6 py-4 animate-pulse">
        ...
        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <div className="h-9 w-9 bg-gray-200 dark:bg-gray-700 rounded-xl" />
            <div className="h-5 w-40 bg-gray-200 dark:bg-gray-700 rounded ml-1" />
          </div>
        ...
        <div className="border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden bg-white dark:bg-gray-900">
          ...
          <div className="grid grid-cols-7 border-b border-gray-100 dark:border-gray-800">
            {WEEKDAYS.map((d) => (
              <div key={d} className="py-3 flex justify-center">
                <div className="h-2.5 w-8 bg-gray-200 dark:bg-gray-700 rounded" />
              </div>
            ))}
          </div>
          ...
          {Array.from({ length: 5 }).map((_, row) => (
            <div key={row} className="grid grid-cols-7 border-b border-gray-100 dark:border-gray-800 last:border-0">
              {Array.from({ length: 7 }).map((_, col) => (
                <div key={col} className="h-24 border-l border-gray-100 dark:border-gray-800 first:border-0 p-2">
                  <div className="h-2.5 w-5 bg-gray-200 dark:bg-gray-700 rounded mb-2" />
                  {/* a couple of event bars on scattered cells */}
                  {(row + col) % 3 === 0 && <div className="h-4 bg-gray-200 dark:bg-gray-700 rounded w-full mb-1" />}
                  {(row + col) % 4 === 0 && <div className="h-4 bg-gray-100 dark:bg-gray-800 rounded w-2/3" />}
                </div>
              ))}
            </div>
          ))}
